...
class Popover extends React.Component {
  ...
  setPosition(props = this.props) {
    ...
    var spaceArrowShouldMove =
      this.state.popoverWidth / 2 - this.state.arrowWidth;

    if (
      props.position >= spaceArrowShouldMove &&
      props.position <= this.props.trackWidth - spaceArrowShouldMove
    ) {
      ...
      this.setState({
        ...
        bubblePosition:
          trackWidth - spaceArrowShouldMove - this.props.handleWidth
      });
    }
  }
  ...
  updatePopoverAndArrowWidth() {
    ...
    var arrowWidth =
      ReactDOM.findDOMNode(this.refs.popover).getElementsByClassName("arrow")
        .length > 0
        ? ReactDOM.findDOMNode(this.refs.popover).getElementsByClassName(
            "arrow"
          )[0].offsetWidth
        : 12;
  ...
  render() {
    var styles = {};
    if (
      isUndefined(this.state.arrowPosition) ||
      isUndefined(this.state.bubblePosition)
    ) {
    ...
    return (
      <div style={styles} ref="container" className="popover-container">
        <div
          ...
          style={popoverStyle}
        >
          <div className="popover__arrow" style={arrowStyle} />
          <div className="popover__content">{this.props.value}</div>
        </div>
      </div>
    );
  }
